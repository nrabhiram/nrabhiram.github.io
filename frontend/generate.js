...
async function readSchema() {
  ...
  return [artifacts, schema, routes];
}

async function createStaticBuildFolder() {
  const staticDirExists = await fs
    .stat(toAbsolute("dist/static"))
    .then(() => true)
    .catch(() => false);
...
function getSecondaryNavData(artifact, includeSummary = false) {
  ...
  secondaryNavData["categories"] =
    artifact.categories || artifact.metadata.categories;
  for (let i = 0; i < nestedArtifacts.length; i++) {
    const nestedArtifact = nestedArtifacts[i];
    const nestedNavItem = getSecondaryNavData(nestedArtifact, includeSummary);
...
function getPagePrimaryNavData(schema) {
  ...
  for (let i = 0; i < primaryRouteKeys.length; i++) {
    ...
    primaryNavKeyAndIndex.push({
      key: keyVal,
      index: schema[keyVal].metadata.index || 0,
    });
  }

  const primaryRoutes = primaryNavKeyAndIndex.sort(
...
function getPageSecondaryNavData(url, schema) {
  ...
  if (isNestedRoute || baseArtifact.artifacts.length > 0)
    secondaryNavData = getSecondaryNavData(
      baseArtifact,
      url.startsWith("/blog") || url.startsWith("/snippets"),
    );
  return secondaryNavData;
}
...
function processSnippetsPageResults(url, results) {
  if (url !== "/snippets") return [];
  const snippetsPosts = results.items || [];
  return snippetsPosts;
}
...
function getPageMetaTags(artifact) {
  ...
  return `
    ...
    <link rel="alternate" type="application/atom+xml" title="Blog Posts, Snippets, and Monthly Logs" href="${BASE_URL}/feed.atom">
...
function createHydrationDataScript(
  primaryNavData,
  secondaryNavData,
  url,
  artifact,
...
function createMasonryHeightScript() {
  return `
    <script>
      function setMasonryHeight() {
        ...
        snippetsContainer.style.height = maxColumnHeight + 'px';
      }
...
function getPageHTML(
  ...
  let snippetsResultsSection = '<div class="snippets">';
  ...
  for (let res of snippetsPostResults) {
    let categoriesSection = "";
  ...
  if (snippetsPostResults.length > 0) content += snippetsResultsSection;
...
function extractFeedEntries(feedArtifacts) {
  ...
  for (const artifact of feedArtifacts) {
    ...
    let contentType = "now";
    if (url.startsWith("/blog")) contentType = "blog";
    if (url.startsWith("/snippets")) contentType = "snippets";
...
async function generateAtomFeed(artifacts) {
  ...
  const feedInfo = {
    title: "Vaxitas - Blog Posts, Snippets, and Monthly Logs",
  ...
  await fs.writeFile(
    toAbsolute("dist/static/feed.atom"),
    utf8BOM + feed,
    "utf8",
  );
...
async function generateSite() {
  try {
    ...
    try {
      ...
      console.error(
        "An error occurred while reading the schema and artifacts:",
        error,
      );
      process.exit(1);
    }
    ...
    for (const url of routes) {
      ...
      const snippetsPostResults = processSnippetsPageResults(
        url,
        secondaryNavData,
      );
      ...
      const isSnippetPost = url.startsWith("/snippets") && url !== "/snippets";

      if (isNowPost || isBlogPost || isSnippetPost)
        feedArtifacts.push(artifact);
