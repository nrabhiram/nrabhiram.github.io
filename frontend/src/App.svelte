...
<Layout
    ...
    prevLink={matchesPath(url, "/now/:slug") && !!artifact?.prev
        ? artifact.prev
        : null}
    nextLink={matchesPath(url, "/now/:slug") && !!artifact?.next
        ? artifact.next
        : null}
>
    ...
    {:else if matchesPath(url, "/now/:slug")}
        <PostPage {artifact} {tableOfContents} />
    {:else if matchesPath(url, "/blog")}
        <BlogHomePage {artifact} />
    {:else if matchesPath(url, "/blog/:slug")}
        <PostPage {artifact} {tableOfContents} />
    {:else if matchesPath(url, "/snipptets")}
        <BlogHomePage {artifact} />
    {:else if matchesPath(url, "/snippets/:slug")}
        <PostPage {artifact} {tableOfContents} />
