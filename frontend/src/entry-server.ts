...
function processImageGrid(content: string) {
  ...
  content = content.replace(imageGridRegex, (match, content: string) => {
    ...
    const gridClass =
      imageData.length < 3
        ? `image-grid col-${imageData.length}`
        : "image-grid col-3";
...
function processSideSections(content: string) {
  const sideParaRegex =
    /__START_PARA_WITH_SIDE_SECTION__([\s\S]*?)__END_PARA_WITH_SIDE_SECTION__/g;
  content = content.replace(sideParaRegex, (match, content: string) => {
    // extract side section content first
    const sectionHeadingMatch = content.match(
      /__START_SIDE_SECTION_HEADING__([\s\S]*?)__END_SIDE_SECTION_HEADING__/,
    );
    const sectionListMatch = content.match(
      /__START_SIDE_SECTION_LIST__([\s\S]*?)__END_SIDE_SECTION_LIST__/,
    );
...
<!-- SECTION_CARD_END -->\n\n`;
    ...
    const sectionHeading = sectionHeadingMatch
      ? sectionHeadingMatch[1].trim()
      : "";
...
function styleSectionCard(content: string) {
  content = content.replace(
    ...
    (match, content) => {
      let sectionHeading =
        content.match(
          /<!-- SECTION_HEADING_START -->([\s\S]*?)<!-- SECTION_HEADING_END -->/,
        )?.[1] || "";
      const sectionList =
        content.match(
          /<!-- SECTION_LIST_START -->([\s\S]*?)<!-- SECTION_LIST_END -->/,
        )?.[1] || "";
...
function styleSideSectionFlexContainer(content: string) {
  content = content.replace(
    ...
    (match, content) => {
      const mainContent =
        content.match(
          /<!-- MAIN_CONTENT_START -->([\s\S]*?)<!-- MAIN_CONTENT_END -->/,
        )?.[1] || "";
      let sideHeading =
        content.match(
          /<!-- SIDE_HEADING_START -->([\s\S]*?)<!-- SIDE_HEADING_END -->/,
        )?.[1] || "";
      const sideList =
        content.match(
          /<!-- SIDE_LIST_START -->([\s\S]*?)<!-- SIDE_LIST_END -->/,
        )?.[1] || "";
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (footnoteStart) {
      ...
      if (currentFootnoteKey && currentFootnoteContent.length > 0) {
        footnotes[currentFootnoteKey] = currentFootnoteContent
          .join("\n")
          .trim();
      }
...
function processNowPageContent(
  url: string,
  content: string,
  ...
  if (!(secondaryNavData.items && secondaryNavData.items.length > 0))
    return content;
...
export async function render(
  ...
  try {
    ...
    processedContent = processNowPageContent(
      url,
      processedContent,
      secondaryNavData,
    );
